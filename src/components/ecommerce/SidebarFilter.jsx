...
const SidebarFilter = ({
  ...
  const [currentMinPrice, setCurrentMinPrice] = useState(minPrice || 0);
  const [currentMaxPrice, setCurrentMaxPrice] = useState(maxPrice || maxProductPrice || 0);
  const debounceTimerRef = useRef(null);

  useEffect(() => {
    setCurrentMinPrice(minPrice || 0);
    setCurrentMaxPrice(maxPrice || maxProductPrice || 0);
  }, [minPrice, maxPrice, maxProductPrice]);
  ...
  const handleRangeChange = ({ min, max }) => {
    ...
    debounceTimerRef.current = setTimeout(() => {
      onPriceChange('min', min);
      onPriceChange('max', max);
    }, 1000);
  };
      ...
      <div className="hidden border-t border-gray-200 pt-6">
        ...
        {expandedSections.price && (
          <div className="space-y-4">
            {maxProductPrice > 0 ? (
              <div className="relative rounded-xl bg-gray-50/50 border border-gray-100 p-3">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-sm font-medium text-gray-900">
                    {formatPrice(currentMinPrice)}
                  </span>
                  <span className="text-sm font-medium text-gray-900">
                    {formatPrice(currentMaxPrice || maxProductPrice)}
                  </span>
                </div>
                ...
                <div className="relative h-6">
                  ...
                  <RangeSlider
                    ...
                    initialMin={minPrice || 0}
                    initialMax={maxPrice || maxProductPrice}
                    onChange={handleRangeChange}
                  />
                </div>

                <div className="flex items-center justify-between mt-2 text-xs text-gray-500">
                  <span>Mín: {formatPrice(currentMinPrice)}</span>
                  <span>Máx: {formatPrice(maxProductPrice || 0)}</span>
                </div>
              </div>
            ) : (
              <div className="relative rounded-xl bg-gray-50/50 border border-gray-100 p-3">
                ...
                <div className="flex items-center justify-between mt-2 text-xs text-gray-500">
                  <span>Mín: {formatPrice(0)}</span>
                  <span>Máx: {formatPrice(0)}</span>
                </div>
              </div>
            )}
          </div>
